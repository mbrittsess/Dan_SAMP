...
local types = {
    ...
    ["int*"        ] = {
        ...
        ArgCheckFetch  = function(List, Name, ArgNum)
            ins(List, fmt(
          [[    %s = (int*)luaL_checkstring(L, %u);]], Name, ArgNum))
        end;
        BaseInvocation = function(List, Name, ArgNum)
            ins(List, fmt(
              [[%s]], Name))
    ...
    ["int*|inarr"    ] = {
        ...
        VariableDefs   = function(List, Name, ArgNum, Argument)
          ...
          [[    int* %s = NULL;]], Name))
            ins(List, fmt(
          [[    int %s = 0;]], Argument.InArr))
        end;
        ArgCheckFetch  = function(List, Name, ArgNum, Argument)
            ins(List, fmt(
          [[    %s = luaconv_OrderedIntArray(L, %u, &%s);]], Name, ArgNum, Argument.InArr))
        end;
    ...
    ["float*|inarr"    ] = {
        ...
        ArgCheckFetch  = function(List, Name, ArgNum, Argument)
            ins(List, fmt(
          [[    %s = luaconv_OrderedFloatArray(L, %u, &%s);]], Name, ArgNum, Argument.InArr))
        end;
        BaseInvocation = function(List, Name, ArgNum, Argument)
            ins(List, fmt(
              [[%s]], Name))
    ...
    ["int*|outstr"]= {
        ...
        VariableDefs   = function(List, Name, ArgNum, Argument)
            ins(List, fmt(
          [[    int* %s = (int*)calloc((size_t)%u, sizeof(char));]], Name, temp_string_buffer_size))
        end;
        ArgCheckFetch  = function(List, Name, ArgNum, Argument)
            ins(List, fmt(
          [[    if (%s == NULL) {luaL_error(L, "Memory-allocation failure for return value '%%s' (tried to allocate %u bytes)", "%s");};]],
        ...
        PushRetVals    = function(List, Name, ArgNum, Argument)
            ins(List, fmt(
          [[    lua_pushstring(L, (char*)%s); free(%s); NumRetVal += 1;]], Name, Name))
        end;
        };
    ["const char*"] = {
        General = {
    ...
    ["int|inarr_len"] = {
        ...
        VariableDefs   = function(List, Name, ArgNum, Argument)
            --[[ins(
                List, 
                fmt(
                    "   int %s = luaconv_OrderedArrayLength(  )",
                    Name
                )
            )]]
        end;
...
ForEach(AMX_Function_Declarations, function(Func)
    ...
    ForEach(Func.Arguments, function(Argument)
        ...
        elseif Argument.InArr then
            Argument.Type    = Argument.Type .. "|inarr"
            local LenArg     = Func.Arguments[Argument.InArr]
            LenArg.Type      = LenArg.Type   .. "|inarr_len"
            LenArg.InArrLen = Argument.Name
        end
    end)
    ...
    ForEach(Func.Arguments, function(Argument)
       ...
       assert(types[Type], string.format("There is no handler implementation for the type of parameter '%s :%s' in function '%s()'", Name, Type, Func.Name))
